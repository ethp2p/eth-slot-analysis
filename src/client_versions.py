...
def main():
    ...
    client_versions_df = query.get_client_versions(db_url, slot_start - 2, slot_end + 2)
    # Save client versions
    client_versions_file_path = os.path.join(out_dir, "client_versions.parquet")
    client_versions_df.to_parquet(client_versions_file_path, index=False)
    logging.info(f"Client versions saved to {client_versions_file_path}")
